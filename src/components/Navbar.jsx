...
function Navbar() {
  ...
  const carsBasket = useSelector((state) => state.carsBasket);
  const totalCount = carsBasket.reduce((acc,curr)=>acc + curr.count,0)

  const totalAmount = carsBasket.reduce((acc,curr)=>acc + (curr.price * curr.count),0)
  console.log(carsBasket);
            ...
            {carsBasket.map((car) => (
              ...
              >
                ...
                <nav className="flex flex-wrap gap-2">
                  ...
                  <button className="text-2xl px-2  border-2 border-white rounded-lg font-semibold" onClick={()=>dispatch(removeCarsBasket(car.id))}>Remove</button>
                </nav>
              </div>
            ))}
          </div>
